...
interface ResultData {
  blNr?: string; // e.g., "SYS2510000000182460"
  schDirCd?: string; // 선박 방향 코드 (E/W 등)
  polCurtEtbDt?: string; // 선적항 예정 접안일시
  polCurtEtaDt?: string; // 선적항 예정 도착일시
  polActlArrvDt?: string; // 선적항 실제 도착일시
  polCurtEtdDt?: string; // 선적항 예정 출항일시
  polCd?: string; // 선적항 코드 (예: KRPUS)
  podLastEtaDt?: string; // 양하항 마지막 ETA
  podActlBerthDt?: string; // 양하항 실제 접안일시
  vvdFullName?: string; // 선박명 + 항차 (예: "UNITED KINGDOM 0124E")
  schVoyNr?: string; // 항차 번호
  svcdLaneCd?: string; // 항로 코드 (예: PSX)
  podActlDeptDt?: string; // 양하항 실제 출항일시
  podCd?: string; // 양하항 코드 (예: CAVAN)
  polActlBerthDt?: string; // 선적항 실제 접안일시
  podActlArrvDt?: string; // 양하항 실제 도착일시
  polNdeNm?: string; // 선적항 터미널명
  polLastEtaDt?: string; // 선적항 마지막 ETA
  vslCd?: string; // 선박 코드
  vvd?: string; // 선박 + 항차 조합 (예: "HJUK0124E")
  polFclCd?: string; // 선적항 시설 코드
  contNr?: string; // 컨테이너 번호
  podCurtEtdDt?: string; // 양하항 예정 출항일시
  polActlDeptDt?: string; // 선적항 실제 출항일시
  podCurtEtbDt?: string; // 양하항 예정 접안일시
  podCurtEtaDt?: string; // 양하항 예정 도착일시
  dspBlNr?: string; // 표시용 BL 번호
	...
	useEffect(() => {
		...
		const startDate = formatDateTime(
			getFirstNonEmpty(data.result, [
				'polActlBerthDt', 'polCurtEtbDt', // 'actlBrthDt', 'currRtbDt'
			]) 
			?? '', 'date', ''
		);
		...
		const endDate = formatDateTime(
			getFirstNonEmpty(data.result, [
				'podActlArrvDt', 'podCurtEtaDt', // 'uastEtaDt', 'currEddDt', 'actlArrvDt'
			]) 
			?? '', 'date', ''
		);
